...
if (mb_strlen($query) < 3 ||
	...
	if (substr_count($query, '▹') == 0) {
		...
		if ($lifetime[22] != "") {
			...
			$subtitle = $subtitle . "Best 🏁 " . $runFarthest . " " . $unit;
		}

		if ($lifetime[47] != "") {
			$subtitle = $subtitle . " ● 🕐 " . formatDuration($lifetime[47], true, false);

		}

		if ($lifetime[48] != "") {
			$subtitle = $subtitle . " ● ⚡️ " . $lifetime[48];
	...
	elseif (substr_count($query, '▹') == 1) {
		...
		if ($kind == "Year") {
			...
			while ($activityByYear = $stmt->fetch()) {
				...
				$Avg_distance = round($distance/$total_activities, 2);

				$title = "YEAR: 🏃 " . $total_activities  . " ● 🏁 " . $distance . " " . $unit;
				$title .= " ● 🕐 ";
				$title .= round($total_duration/1000/60/60, 0) . " hours";
				$title .= " ● 〽️ " . calculatePace($total_duration, $total_distance, $use_miles) . " min/" . $unit . "";
				$subtitle = "⛽️ " . $total_fuel . " ● ⚡️ " . $total_calories . " ● Avg 🏁 " . $Avg_distance . " " . $unit;
			...
			if ($noresult) {
				...
				while ($month_counter >= $end) {
					...
					$year = date('Y', $month_counter);
					$month = date('m', $month_counter);
					//
					...
					if ($old_year != $year) {
						...
						if (!$noresult) {
							...
							$w->result(null, '', $title, $subtitle, './images/' . $year . '.png', 'no', array('copy' => $copytext, 'largetype' => $copytext), "Year▹" . $year . "▹");
						}
					...
					if (!$noresult) {
						...
						$title = "MONTH: 🏃 " . $total_activities  . " ● 🏁 " . $distance . " " . $unit;
						$title .= " ● 🕐 ";
						$title .= round($total_duration/1000/60/60, 0) . " hours";
						$title .= " ● 〽️ " . calculatePace($total_duration, $total_distance, $use_miles) . " min/" . $unit . "";
						$subtitle = "⛽️ " . $total_fuel . " ● ⚡️ " . $total_calories . " ● Avg 🏁 " . $Avg_distance . " " . $unit;
						...
						$w->result(null, '', $title, $subtitle, './images/' . $month . '.png', 'no', array('copy' => $copytext, 'largetype' => $copytext), "Year▹" . $year . "▹" . $month . "▹");
					}
					...
					while ($activity = $stmt->fetch()) {
						...
						$subtitle = "⛽️ " . $activity[26] . " ● ⚡️ " . $activity[28];
		...
		*/
						...
						$title .= $emotion;
						$title .= " ";
						$title .= date("D jS", strtotime($activity[5]));
						$title .= " ● 🏁 ";
						$title .=  $distance;
						$title .= " ";
						$title .= $unit;
						$title .= " ● 🕐 ";
						$title .= formatDuration($activity[29], true, false);
						$title .= " ● 〽️ ";
						$title .= calculatePace($activity[29], $activity[24], $use_miles);
						$title .=  " min/";
						$title .= $unit;

						$nikeurl = 'https://secure-nikeplus.nike.com/plus/activity/running/' . $username . '/detail/' . $activity[0];
						$w->result(null, serialize(array('' /*other_action*/ , $nikeurl /* url */)), $title, $subtitle, './images/' . $activity[17] . '.png', 'yes', array('copy' => 'I ran ' . $distance . " " . $unit . ' on ' . date("M jS", strtotime($activity[5])) . " (" . calculatePace($activity[29], $activity[24], $use_miles) . " min/" . $unit . ") with #nikeplus " . $nikeurl, 'largetype' => $title . "\r" . $subtitle), '');
					}

					$old_year = date('Y', $month_counter);
					$month_counter = strtotime("-1 month", $month_counter);
				}
				...
				while ($shoe = $stmt->fetch()) {
					...
					$w->result(null, '', $shoe[0]  . " ● 🏁 " . $distance . " " . $unit . " ● 🏃: " . $shoe[2], "Retired: " . $retired, './images/shoes.png', 'no', null, '');
	...
	elseif (substr_count($query, '▹') == 2) {
		...
		if ($kind == "Year") {
			...
			if (!$noresult) {
				...
				$w->result(null, '', $title, $subtitle, './images/' . $year . '.png', 'no', array('copy' => $copytext, 'largetype' => $copytext), "");
			}
	...
	elseif (substr_count($query, '▹') == 3) {
		...
		if ($kind == "Year") {
			...
			if (!$noresult) {
				...
				$w->result(null, '', $title, $subtitle, './images/' . $month . '.png', 'no', array('copy' => $copytext, 'largetype' => $copytext), "");
			}
